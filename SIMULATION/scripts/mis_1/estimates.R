...
sapply(1:3000, function(row){
  ...
  point_estimate <- data.frame(
                      true = c(1.078, -4.20753, 0.33348, 1.20088, 0.53604),
  ...
  # Save results
  filename_point <- sprintf(file.path("SIMULATION", "intermediate_results",
   "mis_1", "point_estimates", "point_estimate_%04d_%04d_%04d"), iter, m, uniform_cluster_size)
save("point_estimate", file = sprintf("%s.RData", filename_point))



 filename_interval <- sprintf(file.path("SIMULATION", "intermediate_results", "mis_1",
