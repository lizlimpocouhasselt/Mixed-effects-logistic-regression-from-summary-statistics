#-------------------------------------------------------------------
# MODEL ESTIMATION FOR MODEL MISSPECIFICATION: 
#     MISSPECIFIED RANDOM EFFECTS DISTRIBUTION
#-------------------------------------------------------------------
...
sapply(1:3000, function(row){
  ...
  load(sprintf(file.path("SIMULATION", "intermediate_results", "mis_2", "ps2", 
                            "pseudodata_2ndmom_%04d_%04d_%04d.RData"),
                             iter, m, uniform_cluster_size))
  load(sprintf(file.path("SIMULATION", "intermediate_results", "mis_2", "ps3", 
                            "pseudodata_3rdmom_%04d_%04d_%04d.RData"),
                             iter, m, uniform_cluster_size))
  load(sprintf(file.path("SIMULATION", "intermediate_results", "mis_2", "ps4", 
  ...
  point_estimate <- data.frame(
                      true = c(sqrt(32), -4.20753, 0.33348, -0.25252, 1.20088, 0.53604),
  ...
  # Save results
  filename_point <- sprintf(file.path("SIMULATION", "intermediate_results", "mis_2",
...
save("point_estimate", file = sprintf("%s.RData", filename_point))
 ...
 filename_interval <- sprintf(file.path("SIMULATION", "intermediate_results", "mis_2",
