#-------------------------------------------------------------------
# PSEUDO-DATA (K = 4): GENERALIZED LINEAR MIXED MODEL - LOGIT LINK
#-------------------------------------------------------------------
...
sapply(1:3000, function(row){
    ...
    load(sprintf(file.path("SIMULATION", "intermediate_results", "mis_2", "mv_moment_3_4_bypair_df", 
                            "mv_moment_3_4_bypair_df_%04d_%04d_%04d.RData"), 
                            iter, m, uniform_cluster_size))
    load(sprintf(file.path("SIMULATION", "intermediate_results", "mis_2", "mv_moment_3_4_by3_df", 
                            "mv_moment_3_4_by3_df_%04d_%04d_%04d.RData"), 
                            iter, m, uniform_cluster_size))
    load(sprintf(file.path("SIMULATION", "intermediate_results", "mis_2", "mv_moment_4_df", 
                            "mv_moment_4_df_%04d_%04d_%04d.RData"), 
                            iter, m, uniform_cluster_size))

    # Generate pseudo-data
    pseudodata_4thmom <- fn_pseudodata_4thmom_glmm(iter, m, y_name, uniform_cluster_size, 
    ...
    filename_save <- sprintf(file.path("SIMULATION", "intermediate_results", "mis_2",
     "ps4", "pseudodata_4thmom_%04d_%04d_%04d"), iter, m, uniform_cluster_size)
