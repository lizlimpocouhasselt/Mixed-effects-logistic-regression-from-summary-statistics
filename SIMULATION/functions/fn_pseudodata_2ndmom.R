...
fn_pseudodata_2ndmom_glmm <- function(iter, m, y_name, uniform_cluster_size, seed, names_ind_vars, 
numeric_var_names, mean_cov, var_cov_mat){
  ...
  pseudo_data_2nd_mom <- lapply(1:m, function(group_num){
    ...
    for(name_ind in names_ind_vars){
      ...
      print(paste0('iter ', iter, '; ', 'grp ', group_num, '; ',
       'syn_df_2nd_mom; ', name_ind, ' is finished'))
    }
