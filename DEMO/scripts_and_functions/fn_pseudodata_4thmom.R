...
#   mv_moment_4_df : list of quadvariate 4th central moments
#
# OUTPUT: 
#   dataframe of pseudo-data
#---------------------------------------------------------------

fn_pseudodata_4thmom_glmm <- function(y_name, names_ind_vars, numeric_var_names, mean_cov,
 var_cov_mat, mv_moment_3_4_bypair_df, mv_moment_3_4_by3_df, mv_moment_4_df){
  ...
  pseudo_data_4th_mom <- lapply(1:m, function(group_num){
    ...
    for(name_ind in names_ind_vars){
      ...
      names(syn_df_4th_mom)[ncol(syn_df_4th_mom)] <- name_ind
  ...
  pseudodata_4thmom <- unstd_syn_df_4th_mom
  save(pseudodata_4thmom, file = file.path("DEMO", "ps", "pseudodata_4thmom.RData"))
  
  return(pseudodata_4thmom)
}